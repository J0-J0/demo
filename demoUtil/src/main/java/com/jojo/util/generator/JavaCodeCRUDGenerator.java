...
public class JavaCodeCRUDGenerator {
	...
	public static void generate(String fullyQualifiedName, List<Field> fieldList, String templateFileDirectory,
		...
		// 生成文件名
		StringBuffer basePath = buildBasePath(nameArr);
		...
		// service文件名
		StringBuffer serviceFileName = new StringBuffer(basePath).append("service").append(File.separator);
		serviceFileName.append(classSimpleName).append("Service.java");
		// serviceImpl文件名
		StringBuffer serviceImplFileName = new StringBuffer(basePath).append("service").append(File.separator);
		serviceImplFileName.append("impl").append(File.separator).append(classSimpleName).append("ServiceImpl.java");
		// jsp文件名
		StringBuffer jspFileName = new StringBuffer(PROJECT_DIRECTORY).append(File.separator);
	...
	private static StringBuffer buildBasePath(String[] nameArr) {
		/*
		 * 只考虑规范的全限定类名，即最后一个是类的名字，倒数第二个是实体类的包名。
		 */
		StringBuffer basePath = new StringBuffer(PROJECT_DIRECTORY);
